...
import type { Env, EnvAny, EnvBoolean, EnvInteger, EnvString } from './types'

export function envParseInteger(key: EnvInteger, defaultValue?: number): number {
  const value = process.env[key]
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined) {
      throw new Error(`[ENV] ${key} - The key must be an integer, but is empty or undefined.`)
    }
    return defaultValue
  }
...
export function envParseBoolean(key: EnvBoolean, defaultValue?: boolean): boolean {
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined) {
      throw new Error(`[ENV] ${key} - The key must be a boolean, but is empty or undefined.`)
    }
    return defaultValue
  }
...
export function envParseString<K extends EnvString>(key: K, defaultValue?: Env[K]): Env[K] {
  ...
  if (isNullishOrEmpty(value)) {
    if (defaultValue === undefined) {
      throw new Error(`[ENV] ${key} - The key must be a string, but is empty or undefined.`)
    }
    return defaultValue
  }
